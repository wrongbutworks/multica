...
import { useCreateTeam } from "@multica/core/teams/mutations";
...
export function CreateTeamModal({ onClose }: { onClose: () => void }) {
  ...
  const keyError = keyTouched && !TEAM_KEY_REGEX.test(normalizedKey);
  const canSubmit =
    name.trim().length > 0 && TEAM_KEY_REGEX.test(normalizedKey) && !submitting;
