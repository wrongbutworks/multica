...
Agent 的 Space access 同时是数据授权边界，而不只是界面可见性：Agent 加入某个 Space 后，才有资格在该 Space 工作和访问该 Space 数据；每次运行仍绑定唯一 Space，Direct Chat 没有 Space 上下文时不具备任何 Space 数据权限。
...
```text
Workspace Context
  + 当前运行唯一 Space 的 Space Context
  + 该 Space 的 Integration bindings
  + Project resources
  + Issue-specific context
  ∩ 当前成员或 Agent 的实际权限
```

只有绑定了 Space 的 Issue、Autopilot 和 Quick Create 运行会收到 Space Context；无 Space 的 Direct Chat 不读取任何 Space Context。Context 只提供工作说明，不授予数据、Secret 或第三方 Integration 权限。
