...
> 更新时间：2026-07-11。
>
> 范围：定义 Workspace、Space、Project、Issue、Agent、Skill、Squad、Autopilot 和未来跨 Space 协作层之间的产品关系。本文同时记录目标模型和分阶段实现状态。
...
截至 2026-07-11，Phase 1 至 Phase 3 的目标模型已经落入代码：

- **Phase 1 已完成**：Project 和 Issue 强制单 Space，Project 内 Issue、parent issue 与 sub-issue 必须同 Space；旧跨 Space 数据通过迁移拆分，Sidebar 不再提供 Workspace 级 Projects 入口。
- **Phase 2 已完成**：Default Space 可在 Workspace Settings 配置；Open / Private Space、Space 角色和个人 Pin / 排序已经分离；Agent 支持 Private / Selected Spaces / Workspace 的 Space access，Skill 支持独立分享范围。
- **Phase 3 已完成**：Squad 和 Autopilot 单 Space 化，提供 Workspace 级 Autopilot Template；Integration connection 可以显式绑定 Space；Workspace Context 与 Space Context 按运行边界分层注入；Space 归档、恢复、Autopilot 暂停与 Settings 中可见的活动记录形成完整生命周期。
- **明确延后**：不提供 Move Project、Move Issue；跨 Space 目标层 Initiative 留到 Phase 4，等待真实需求和使用数据。

Agent 的 Space access 同时是数据授权边界，而不只是界面可见性：Agent 加入某个 Space 后，才有资格在该 Space 工作和访问该 Space 数据。Issue、Autopilot、Quick Create 等工作运行仍绑定唯一 Space；Chat 则由用户在输入框上方选择一个 Space 或 All spaces，并且只能访问「成员可协作的 Space ∩ Agent 可用的 Space」。
...
- Private：只有 Owner 可以调用；运行可使用的 Space 仍不能超过 Owner 自己可协作的范围；
- Selected Spaces：Agent 明确加入若干 Space，可在这些 Space 中被发现、@mention、分配和调用；
- Workspace：Agent 默认加入当前及未来所有 Active Space。

把 Agent 加入一个 Space，会授予它在该 Space 执行工作所需的数据访问，但不会给 Agent Owner 或调用者增加任何人类权限。Issue、Autopilot、Quick Create 等任务令牌只绑定当前唯一 Space，Space A 的运行不能读取 Space B。Chat 是显式例外：新会话可以选择一个 Space，或选择 All spaces 访问成员与 Agent 共同可用的全部 Space；All spaces 是可见、可修改的上下文选择，不代表 Workspace 全量权限。

因此一次执行的有效范围是「成员的 Space collaboration access ∩ Agent 的 Space access ∩ 会话/任务选择的 Space scope ∩ 该任务允许使用的 Integration / Resource」。Agent 只有在目标 Space 可用时，才能成为该 Space Issue、Autopilot 或 Squad 的执行者；运行期间移除成员或 Agent 的 Space access，应立即撤销该令牌后续的 Space 数据访问。
...
- Space Context：该 Space 中所有运行共享的领域知识、规范和责任边界；
...
```text
Workspace Context
  + 当前运行所选一个或多个 Space 的 Space Context
  + 所选 Space 的 Integration bindings
  + Project resources
  + Issue-specific context
  ∩ 当前成员或 Agent 的实际权限
```

Issue、Autopilot 和 Quick Create 收到其唯一 Space 的 Context。Chat 收到创建会话时显式选择的单个 Space Context，或 All spaces 范围内每个 Space 的 Context；会话创建后范围保持不变。Context 只提供工作说明，不授予额外数据、Secret 或第三方 Integration 权限。
...
9. Agent 加入 Space 只授予该 Space 的数据权限；工作运行绑定一个 Space，Chat 可显式选择一个或 All spaces；Integration 仍需显式 binding。分享 Skill 不授予数据或 Integration 权限。
...
- 为 Agent 增加 Private / Selected Spaces / Workspace 的 Space access；工作运行绑定唯一 Space，Chat 提供单 Space / All spaces 上下文选择；
