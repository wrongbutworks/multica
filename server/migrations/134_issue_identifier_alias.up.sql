-- Issue identifier aliases: when an issue moves to another team it is
-- renumbered (numbers are per-team), so its old identifier (e.g. MUL-3)
...
CREATE TABLE issue_identifier_alias (
    ...
    team_key_lower TEXT NOT NULL,
    number INTEGER NOT NULL,
    issue_id UUID NOT NULL REFERENCES issue(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (workspace_id, team_key_lower, number)
);
